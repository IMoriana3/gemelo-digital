...
Lo mide la **HSU**. Y la HSU no habla con nadie por su cuenta: la **NCU es el maestro** de la Zigbee y la **sondea**. La NCU tampoco empuja nada al vuelo — alcanza a cada TCU cuando le toca. Así que entre que sopla y que un seguidor lo sabe hay una espera, no cero:

```
ráfaga → [la HSU la mide] → [la NCU la lee en su ranura] → [el TCU la recibe en la suya] → abandera
```
...
- **El deslizador de hora COLOCA la planta, no simula el camino.** A 0,17 °/s cruzar los 110° son once minutos: si el deslizador simulara el transitorio, el campo iría siempre persiguiendo un objetivo que ya cambió. Al arrastrar, cada mesa se pone en su objetivo — y ocurre en el `input`, mientras arrastras, no al soltar. Con **dos excepciones**, que son el motivo de que exista este simulador: el equipo con el **motor cortado** no se mueve (con la seta pulsada la mesa se queda donde está mientras el algoritmo sigue calculando por debajo), y al **inclinómetro** no se le copia el ángulo — se le asienta el filtro y sigue midiendo lo suyo, así que si va descalibrado sigue publicando el ángulo torcido.
- **Elegir un emplazamiento configura la planta.** Los seguidores, las estaciones, los repetidores (los `tcuSinMesa` del layout), los grupos y el huso salen del layout del DWG, no de lo que hubiera en las casillas. Y el campo 3D coloca **cada seguidor donde está en el plano**, con su tipo (completo o medio) y el azimut de su eje. El punto del layout es el eje de unidad —el centro de la bífila, no el motor—, y llega en [norte, este]: aquí el norte es −X y el este −Z, así que se le da la vuelta a los dos. Equivocar ese signo pone la planta en espejo y no canta hasta que alguien la compara con el plano.
- **El testigo de salud va sobre el MOTOR**, que es donde está la TCU cuya salud representa: centro del tubo, viga oeste. Estaba en el extremo de la fila, que es un sitio sin nada — y con dos vigas por seguidor, un punto flotando en una punta no dice de quién es. La viga del motor va al **oeste**, como dice el layout: *«el motor va en la viga OESTE, a filaZ al oeste del eje de unidad, y ahí es donde va la TCU»*. En este marco el norte es −X y el este −Z, o sea que el oeste es **+Z**.
...
- **El eje de transmisión y los ajustes de sombra no se han inventado aquí**: las medidas del eje (Ø 60 mm, 22 cm bajo el eje del tubo, cardanes de Ø 180 × 300) salen de `overcast.html` y están **confirmadas**; el criterio de sombras —recuadro ceñido a la vista y ANISÓTROPO con el sol rasante— es el de `backtracking.html`, que ya dejó escrito por qué salía en sierra. Cuidado: `overcast.html` lleva dos ejes distintos, Ø 60 en la vista instanciada y Ø 100 en la de detalle. El bueno es el Ø 60.

...
- **Simulador de planta — gemelo total (`simulador.html`):** la planta entera equipo a equipo (**NCU + TCUs + HSUs + repetidores**) con la **jerarquía de control real** —seta > SP1 viento > SP3 nieve > SP4 limpieza > forzados > restricción de batería > manual > auto— y el **mapa Modbus de los tres dispositivos en vivo**: cada registro con su valor crudo, decodificado y sus bits activos, actualizándose paso a paso, con destello en lo que cambia y export a CSV. El mapa es el canónico (NCU R7 · TCU v6 · HSU R23), generado de la ficha de `cobertura-zigbee` por `tools/extrae_mapa.mjs` — 515 direcciones, sin copiar ninguna a mano. El **algoritmo y la gestión de batería no se copian: se llaman**. Con el motor local de SolarGPT levantado (`SolarGPTfull/server`), `POST /tracker` da la trayectoria del día —backtracking y política de cielo cubierto dentro— y `POST /tcubalance` el balance de batería sobre los ángulos que el gemelo ha ejecutado. Sin motor la página abre igual, pero lo dice: el indicador pasa a «navegador» y avisa de que eso es un modelo de primer orden. Las constantes tampoco se copian. `sim/fisica.js` es un **espejo vigilado** del core: se mantiene a mano declarando que el Python manda, y `tools/carea_fisica.mjs` lo carea a 1e-9 contra los goldens que genera SolarGPT (`solargpt/scripts/genera_goldens_fisica.py`). Cubre hoy 3 de sus 6 funciones y **lo imprime en cada ejecución**, porque un arnés que cubre la mitad sin decirlo convierte su verde en «todo careado». Sobre eso corre la estrategia oficial SUNNER equipo por equipo: techo de SOC objetivo, carga completa periódica, defensa por SOC crítico con rearme +2 % y su contador de no disponibilidad, y el winter mode canónico (techo 90 % y calibración cada 3 días, y nada más: no toca el seguimiento). Todos los parámetros se tocan en caliente. Las **entradas físicas** están modeladas con su naturaleza: el **inclinómetro es analógico** —el lazo se cierra sobre lo que MIDE el TCU, con su desajuste de montaje, su offset de 41058, su deriva y su cuantización a 34,7 pulsos/°, así que se puede simular el TCU que publica 0° con la mesa a 3° y el SCADA en verde—, y la **seta es binaria**: línea normalmente cerrada con antirrebote y **enclavada** (soltarla no rearma; hace falta 40007.13), que corta el puente en H mientras el algoritmo sigue calculando por debajo. El eje calado y el eje duro son averías físicas que el firmware **deduce** por sobrecorriente o por reintentos, no banderas que se declaren. Y el criterio de salud es el del SCADA. Y el mapa **sale por el cable**: `sim/servidor.mjs` corre el mismo motor en Node y lo publica por HTTP, y `scada/tools/ncu_simulada.py --gemelo` —un esclavo Modbus TCP de verdad— lo sirve, de modo que el colector y la TCU Toolbox reales hablan con la planta simulada. La escritura vuelve por el mismo sitio: un FC06/FC16 contra esa NCU entra por la misma puerta que usa la interfaz, así que un forzado de posición segura escrito desde la toolbox mueve la planta. Motor sin DOM en `sim/planta.js`, con prueba de humo: `node sim/prueba.mjs` (196 comprobaciones). Detalle en [`sim/README.md`](sim/README.md).
- **Simulador de gestión de batería (`bateria.html`):** balance energético del TCU hora a hora durante **todo el año**, con la metodología del estudio de disponibilidad de batería. Meteo: año histórico completo de Open-Meteo, previsión 3–16 días, **CSV horario de PVSyst** (usa GlobEff/POA real, T_Amb, DiffHor y el ángulo real del tracker PhiAng; el fichero no sale del navegador) o sintética hasta 365 días. Física canónica de SolarGPT (00.2t + 11.5b): consumos medidos (electrónica 0,64 W; motor Wh/° = 0,0503 + 0,000845·|θ|) o consumo medio en mA del estudio de disponibilidad (2500/3250/4000 @25,6 V), carga limitada por Tª mínima y C-rate seguro LiFePO₄. Estrategia SOC oficial (objetivo 80 % / crítico 30 % → defensa 55° / carga completa cada 5 días) + winter mode canónico (90 %/3 días). Perfiles con variantes **LT calefactadas** (P = 1 + 0,15·|T| W bajo 0 °C, desbloquea la carga) y JEITA en el lado caliente (reduce a 35 °C, bloquea a 45 °C). **Abanderamientos por viento** (estrategia B2: parcial ≥40 km/h, full ≥60 km/h, histéresis) con su consumo de motor y su banda en las gráficas. Emplazamientos preset = plantas Factiun con su nº de proyecto (coords reales de los layouts). Fuente de carga según el tipo de TCU: en los **SP** el panel dedicado va **solidario al seguidor** (ve el plano del generador, y su captación cae cuando el seguidor está en defensa o abanderado), mientras que los **String** toman su potencia nominal de un string de varios kW y disponen de ella desde muy poca luz — esa diferencia es la que explica que los modelos String salgan mucho mejor. Métricas de impacto del estudio: **% tiempo no disponible** y **% producción perdida** (POA defensa vs seguimiento), rainflow counting (ciclos ≥30 %/≥60 % DoD), balance mensual, y vida a 80 % SOH con el envejecimiento desglosado en ciclado (Nf = 6000/DoD^1,2) y calendario integrado paso a paso con el SOC y la temperatura de celda (Arrhenius × factor de SOC del BatteryModel v2 canónico), más la comparativa con/sin estrategia. Incluye una **comparativa entre sistemas de control** (sin gestión, estrategia estándar, winter mode, defensa en plano y conservador) sobre la misma meteo, con la producción relativa al seguimiento ideal para que el ahorro de un modo no oculte lo que cuesta en producción, y un **generador de informe** imprimible en A4 con la estructura del estudio de disponibilidad (objeto y conclusión, procedimiento, emplazamiento, meteorología, especificación de batería y motor, estrategia SOC, métricas de impacto, resultados año a año con media y percentil 97,5 %, comparativa de controles, ciclado e hipótesis).

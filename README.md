...
- **Simulador de planta — gemelo total (`simulador.html`):** la planta entera equipo a equipo (**NCU + TCUs + HSUs + repetidores**) con la **jerarquía de control real** —seta > SP1 viento > SP3 nieve > SP4 limpieza > forzados > restricción de batería > manual > auto— y el **mapa Modbus de los tres dispositivos en vivo**: cada registro con su valor crudo, decodificado y sus bits activos, actualizándose paso a paso, con destello en lo que cambia y export a CSV. El mapa es el canónico (NCU R7 · TCU v6 · HSU R23), generado de la ficha de `cobertura-zigbee` por `tools/extrae_mapa.mjs` — 515 direcciones, sin copiar ninguna a mano. Modela las tres **alimentaciones** (SP autoalimentado, STRING por regulador, AC con batería de respaldo) y sus variantes LT calefactadas, las averías que se provocan a mano (eje bloqueado, TCU muda, seta local, corte de alterna) y el criterio de salud del SCADA. Motor sin DOM en `sim/planta.js`, con prueba de humo: `node sim/prueba.mjs`. Detalle en [`sim/README.md`](sim/README.md).
